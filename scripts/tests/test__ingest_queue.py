...
TEST_AUDIO_FILE_2 = "../media/media/unit-test-data/Treasures/01 Creativity & Initiative.mp3"
...
class TestIngestQueue(unittest.TestCase):
    ...
    def test_add_item(self):
        ...
        self.assertTrue(os.path.exists(os.path.join(self.test_queue_dir, f"{item_id}.json")))
    ...
    def test_get_next_item(self):
        # Add a few items
        item1_id = self.queue.add_item('audio_file', {'file_path': TEST_AUDIO_FILE_1, 'author': TEST_AUTHOR, 'library': TEST_LIBRARY})
        item2_id = self.queue.add_item('youtube_video', {'url': TEST_YOUTUBE_URL, 'author': TEST_AUTHOR, 'library': TEST_LIBRARY})
        item3_id = self.queue.add_item('audio_file', {'file_path': TEST_AUDIO_FILE_2, 'author': TEST_AUTHOR, 'library': TEST_LIBRARY})
    ...
    def test_update_item_status(self):
        item_id = self.queue.add_item('audio_file', {'file_path': TEST_AUDIO_FILE_1, 'author': TEST_AUTHOR, 'library': TEST_LIBRARY})
    ...
    def test_remove_item(self):
        ...
        self.assertFalse(os.path.exists(os.path.join(self.test_queue_dir, f"{item_id}.json")))

    def test_get_queue_status(self):
        self.queue.add_item('audio_file', {'file_path': TEST_AUDIO_FILE_1, 'author': TEST_AUTHOR, 'library': TEST_LIBRARY})
        item_id = self.queue.add_item('youtube_video', {'url': TEST_YOUTUBE_URL, 'author': TEST_AUTHOR, 'library': TEST_LIBRARY})
        self.queue.add_item('audio_file', {'file_path': TEST_AUDIO_FILE_2, 'author': TEST_AUTHOR, 'library': TEST_LIBRARY})
