...
def delete_records_by_prefix(index, prefix):
    ...
    confirmation = input(
        f"Are you sure you want to delete {len(record_ids)} records with prefix '{prefix}'? (yes/no): "
    ).lower()
    if confirmation == "yes":
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Delete Pinecone records by file prefix for audio or video files."
    )
    parser.add_argument(
        "file_path", type=str, help="Path to the media file (audio or video)"
    )
    parser.add_argument(
        "--library", type=str, required=True, help="Name of the library"
    )
    parser.add_argument(
        "--file-type",
        type=str,
        ...
        help="Type of the file (audio or youtube)",
    )
